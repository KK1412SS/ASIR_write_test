...
# Import real median-path data from AnimCJK as an alternate Chinese source
python3 import_animcjk_data.py --variant zhhans --text "你好中文一二三" --skip-missing --max-workers 1 --timeout 90 --retries 3
python3 import_animcjk_data.py --variant zhhant --text "你好中文一二三" --skip-missing --max-workers 1 --timeout 90 --retries 3
...
# Direct writing with AnimCJK as the actual Hanzi source.
# Missing Hanzi are now auto-imported from AnimCJK, while fallback is limited to punctuation.
...
# Alternate real source names now available in the GUI and Python API:
# - hanziwriter
# - animcjk_zhhans
# - animcjk_zhhant
