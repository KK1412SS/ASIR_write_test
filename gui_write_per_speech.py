...
def get_supported_chars_text():
    """
    Baut automatisch eine Anzeige aus allen Zeichen,
    die aktuell in LETTER_STROKES vorhanden sind.
    """
...
def validate_drawable_text(text):
    """
    Prüft, ob jedes Zeichen im aktuellen LETTER_STROKES vorhanden ist.

    Wichtig:
    - Buchstaben werden automatisch als Großbuchstaben geprüft.
    - Leerzeichen und Newline sind erlaubt.
    - Alle neuen Symbole aus LETTER_STROKES werden automatisch unterstützt.
    """
    unsupported = []
...
def normalize_draw_text(text):
    """
    Für dein aktuelles LETTER_STROKES-System:
    - Buchstaben werden groß.
    - Zahlen und Symbole bleiben gleich.
...
def draw_worker(text):
    ...
    try:
        is_drawing = True
        set_status(f"机器人确认要写：{text}\nRobot confirmed writing: {text}")
...
def start_writing_callback(sender=None, app_data=None):
    ...
    if unsupported:
        ...
        set_status(
            "这些字符目前只能显示，不能绘制：\n"
            + " ".join(unsupported)
            + "\n\n当前 LETTER_STROKES 支持：\n"
            + get_supported_chars_text()
            + "\n\nThe GUI can display Chinese, but the robot stroke library cannot draw it yet."
        )
        return
